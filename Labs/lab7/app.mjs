...
let toggleTask = (req, res) => {
    // get taskitem by id and change the status
    let taskItemId = req.params.taskId; 
    for (let i = 0; i < tasks.length; i++){
        if (tasks[i].id == taskItemId){
            tasks[i].status = 1 - tasks[i].status;
            break;
        }
    }
    res.redirect('/');
...
router.route('/toggle/:taskId').get(toggleTask);
router.route('/api/tasks/:taskId').get(listSingleTask);
router.route('/new').get(newTask);
router.route('/delete/:taskId').get(removeTask);
